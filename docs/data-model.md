...
So the increment is not left to callers. It happens once, centrally, in `UnitOfWork`:
...
Forgetting to bump a version is now impossible: any modified entity implementing `IVersioned` gets one.
...
cannot lock them in opposite directions and deadlock — and saves. The central loop above supplies the
