...
| Tests | xUnit + FluentAssertions + `Testcontainers.MsSql` + NSubstitute | |
...
**Kept:** `IUnitOfWork` (repositories do not commit; the handler decides the transaction boundary) and
`IClock` (the only way to write a deterministic test for the ninety-day booking horizon).
...
│   │   ├── Persistence/
│   │   │   ├── AppointmentDbContext.cs # mapping only — no migrations, no seeding
│   │   │   ├── UnitOfWork.cs
...
`ExecuteInTransactionAsync` also hides EF Core's execution-strategy requirement, so the Application
layer never learns that retries exist.
...
`IsConcurrencyToken()`, and `UnitOfWork.SaveChangesAsync` increments it for every modified `IVersioned`
entity:

```csharp
foreach (var entry in _db.ChangeTracker.Entries<IVersioned>()
...
A plain `int` protects nothing unless it is bumped on every write, and leaving that to callers
guarantees someone eventually forgets. Doing it once, centrally, closes the hole before it exists.
