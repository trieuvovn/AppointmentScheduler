...
public class DealershipTests
{
    ...
    private static readonly TimeZoneInfo PositiveOffsetZone = FindTimeZone("Asia/Ho_Chi_Minh", "SE Asia Standard Time");

    // A zone that observes DST, for the transition cases.
    private static readonly TimeZoneInfo DstObservingZone = FindTimeZone("Europe/London", "GMT Standard Time");
    ...
    public void OpeningWindowOn_DayWithHoursRecorded_ResolvesLocalHoursAgainstTheDealershipTimeZone()
    {
        ...
        var window = OpenWeekdays().OpeningWindowOn(Monday, PositiveOffsetZone);
    ...
    {
        OpenWeekdays().OpeningWindowOn(Sunday, PositiveOffsetZone).Should().BeNull();
    }
    ...
    {
        var slot = LocalSlot(Monday, 10, 60, PositiveOffsetZone);

        OpenWeekdays().IsWithinOpeningHours(slot, Monday, PositiveOffsetZone).Should().BeTrue();
    }

    [Fact]
    public void IsWithinOpeningHours_SlotStartingExactlyAtOpening_ReturnsTrue()
    {
        var slot = LocalSlot(Monday, 8, 60, PositiveOffsetZone);
    ...
    {
        var slot = LocalSlot(Monday, 16, 60, PositiveOffsetZone);
    ...
    {
        var slot = LocalSlot(Monday, 7, 60, PositiveOffsetZone);

        OpenWeekdays().IsWithinOpeningHours(slot, Monday, PositiveOffsetZone).Should().BeFalse();
    }

    [Fact]
    public void IsWithinOpeningHours_ServiceThatCrossesClosingTime_ReturnsFalse()
    {
        var slot = LocalSlot(Monday, 16, 90, PositiveOffsetZone);
    ...
    {
        var slot = LocalSlot(Sunday, 10, 60, PositiveOffsetZone);

        OpenWeekdays().IsWithinOpeningHours(slot, Sunday, PositiveOffsetZone).Should().BeFalse();
    }
    ...
    public void OpeningWindowOn_DatesEitherSideOfADstChange_FollowsTheOffsetInForceOnEachDate()
    {
        ...
        var winterWindow = dealership.OpeningWindowOn(winter, DstObservingZone)!.Value;
        var summerWindow = dealership.OpeningWindowOn(summer, DstObservingZone)!.Value;
